...
- **Overview** — fleet/contract/wake/LLM tiles, credits chart, the agent's current thought, alerts (low fuel, deadlines, failing wakes, LLM errors), recent wakes, and the server leaderboards (most credits, most charts) with your position. The leaderboard is for you only; the agent never sees it.
...
- **Galaxy** — every system in the universe plotted by position and colored by star type, fleet systems ringed with ship counts, known jump-gate links, ships jumping or warping between systems, search by symbol; click a system for detail, double-click to open its system map. The system list is fetched once per reset in the background (the bulk `/systems.json` dump if the server offers it, else `GET /systems` pages while the agent sleeps) and never shown to the model.
- **System** — pan/zoom system map: waypoint types, markets, orbitals, parked ships and in-transit ships moving along their routes.
