...
Env knobs (optional): `AGENT_POLICY=readonly` (block all mutating tools), `AGENT_MAX_ACTIONS_PER_WAKE`, `AGENT_MAX_ROUNDS_PER_WAKE`, `AGENT_MAX_CONCURRENT_TOOLS`, `AGENT_FALLBACK_WAKE_MS`, `AGENT_MIN_WAKE_GAP_MS`, `PANEL_PORT`, `PANEL_HOST` (default 127.0.0.1), `TRANSPORT_MIN_INTERVAL_MS`, `TRANSPORT_TIMEOUT_MS`, `LLM_TIMEOUT_MS`.

## Panel

`http://127.0.0.1:8787` — localhost-only (Host/Origin validated). Plain HTML/CSS/JS in `src/panel/public/`, no build step; live over one SSE stream. An always-visible status strip shows whether the agent is executing (wake/round), standing by (next wake countdown) or paused, plus credits, API budget and LLM token use.
