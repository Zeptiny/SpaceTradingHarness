...
import type { Contract, System, Waypoint } from "../generated/types.js";
...
export function startPanel(): void {
  ...
  app.get("/api/markets/history", (req, res) => {
    const q: { good?: string; waypoint?: string; limit?: number } = {};
  ...
  app.get("/api/activity", (req, res) => {
    const q: { tool?: string; outcome?: ToolOutcome; since?: number; limit?: number } = {};
    ...
    const limit = Number(req.query.limit);
    if (Number.isFinite(limit) && limit > 0 && limit <= 500) q.limit = limit;
    res.json(activity.query(q));
  });

  app.get("/api/summaries", (_req, res) => res.json(summaries.recent(30).reverse()));
